...
impl Display for Op {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Op::Rol { src, dst } => write!(f, "rol {},{}", dst, src),
            Op::Ror { src, dst } => write!(f, "ror {},{}", dst, src),
            Op::Rcl { src, dst } => write!(f, "rcl {},{}", dst, src),
            Op::Rcr { src, dst } => write!(f, "rcr {},{}", dst, src),
            Op::Shl { src, dst } => write!(f, "shl {},{}", dst, src),
            Op::Shr { src, dst } => write!(f, "shr {},{}", dst, src),
            Op::Sar { src, dst } => write!(f, "sar {},{}", dst, src),
